      ...
      <div style={{ padding: "16px 24px", borderBottom: `1px solid ${C.rule}`, flexShrink: 0 }}>
        ...
        <div
          ...
          style={{
            marginTop: 10,
            padding: "10px 12px",
            border: `1px solid ${hub.health?.ok ? "rgba(61,200,122,.35)" : C.rule}`,
            fontSize: ".38rem",
            color: C.ink4,
            letterSpacing: ".06em",
            lineHeight: 1.6,
        ...
        >
          <strong style={{ color: C.vg }}>Super Agents hub</strong>
          {hub.loading ? " · loading…" : hub.health?.ok ? " · online" : hub.error ? ` · ${hub.error}` : " · set VITE_SUPER_AGENTS_HUB_URL"}
          {hub.inventory?.daytona?.sandboxes?.length != null ? (
            <> · Daytona: {hub.inventory.daytona.sandboxes.length} sandbox(es)</>
          ) : null}
          {hub.inventory?.agents?.length ? (
            <> · agents: {hub.inventory.agents.map((a) => a.name).join(", ")}</>
          ) : null}
        </div>
      </div>
